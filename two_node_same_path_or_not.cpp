...
///////////////////////////////////////////////////////////////////////
int t = 0;
void markinout(int root, vector<vector<int>> &graph, vector<int> &in, vector<int> &out, vector<int> &visited, int n)
{
    visited[root] = 1;
    t++;
    in[root] = t;
    for (auto x : graph[root])
    {
        if (!visited[x])
        {
            markinout(x, graph, in, out, visited, n);
        }
    }
    t++;
    out[root] = t;
...
*/
    ...
    markinout(1, graph, in, out, visited, n);
    int q;
    cin >> q;
    while (q--)
    {
        int x, y;
        cin >> x >> y;
        if ((in[x] < in[y] and out[y] < out[x]) or (in[x] > in[y] and out[y] > out[x]))
            cout << "YES:)" << endl;
        else
        {
            cout << "NO:(" << endl;
